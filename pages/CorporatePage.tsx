...
const Word: React.FC<{ word: string, index: number, isMobile: boolean }> = ({ word, index, isMobile }) => {
...
const ExecutiveDossier: React.FC<{ isMobile: boolean }> = ({ isMobile }) => {
    ...
    const executives = [
        { id: 1, name: "Emrullah Malkoç", role: "Operasyon Müdürü", img: "/images/team/emrullah-malkoc.jpg", bio: "15+ yıllık deneyimle operasyonel mükemmelliği yönetiyor.", objectPosition: "center center" },
        { id: 2, name: "Tuncer Beyazoğlu", role: "Elektrik Sistemleri Direktörü", img: "/images/team/tuncer-beyazoglu.jpg", bio: "Tüm elektrik altyapısı koordinatörü ve proje mühendisi.", objectPosition: "20% 40%" },
        { id: 3, name: "Berhudan Başcan", role: "Bilgisayar Mühendisi", img: "/images/team/berhudan-bascan.jpg", bio: "Otomasyon yazılımları ve dijital dönüşüm proje mühendisi.", objectPosition: "center 20%" },
    ];
                        ...
                        {executives.map((exec, i) => (
                            ...
                            >
                                <div className="h-64 overflow-hidden relative bg-zinc-950">
                                    <img
                                        src={exec.img}
                                        className="w-full h-full object-contain filter grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-700"
                                        alt={exec.name}
                                        onError={(e) => {
                                            console.error('Image failed to load:', exec.img);
                                            (e.target as HTMLImageElement).src = 'https://via.placeholder.com/300x400?text=' + encodeURIComponent(exec.name);
                                        }}
                                    />
